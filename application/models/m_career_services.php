...
class M_career_services Extends CI_Model
{
	private $_table = 'career_services';
	...
	public function create_career_services($input = false)
	{
		$this->db->insert($this->_table,$input);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function get_career_services($id = false)
	{
		$sql = "select *
				FROM $this->_table
				WHERE id = ?
	...
	public function update_career_services($data = false, $where = false)
	{
		$this->db->set($data)->where('id',$where)->update($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_career_services($where = false)
	{
		$this->db->where('id',$where)->delete($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_career_services_by_employee_id($where = false)
	{
