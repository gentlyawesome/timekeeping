...
class M_remarks Extends CI_Model
{
	...
	public function create_remarks($input = false)
	{
		$this->db->insert($this->_table,$input);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function get_remarks($id = false)
	{
		$sql = "select *
				FROM $this->_table
				WHERE id = ?
	...
	public function update_remarks($data = false, $where = false)
	{
		$this->db->set($data)->where('id',$where)->update($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_remarks($where = false)
	{
