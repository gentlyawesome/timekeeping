...
class M_lessons Extends CI_Model
{
  ...
  public function get($id = false,$array = false)
	...
	public function create_lesson($input = false)
	{
		$this->db->insert($this->_table,$input);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function get_lesson($id = false)
	{
		$sql = "select *
				FROM $this->_table
				WHERE id = ?
	...
	public function get_lessons_of_employee($id = false)
	{
	  $sql = "select *
				FROM $this->_table
				WHERE user_id = ?
	...
	public function update_lesson($data = false, $where = false)
	{
		$this->db->set($data)->where('id',$where)->update($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_lesson($where = false)
	{
