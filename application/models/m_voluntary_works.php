...
class M_voluntary_works Extends CI_Model
{
	private $_table = 'voluntary_works';
	...
	public function create_voluntary_works($input = false)
	{
		$this->db->insert($this->_table,$input);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function get_voluntary_works($id = false)
	{
		$sql = "select *
				FROM $this->_table
				WHERE id = ?
	...
	public function update_voluntary_works($data = false, $where = false)
	{
		$this->db->set($data)->where('id',$where)->update($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_voluntary_works($where = false)
	{
		$this->db->where('id',$where)->delete($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_voluntary_works_by_employee_id($where = false)
	{
