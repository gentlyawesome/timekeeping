...
class M_student_grade_files Extends CI_Model
{
	private $_table = 'student_grade_files';
	...
	public function create_student_grade_files($input = false)
	{
	...
	public function get_student_grade_files($id = false)
	{
		$sql = "select *
				FROM $this->_table
				WHERE id = ?
	...
	public function update_student_grade_files($data = false, $where = false)
	{
		$this->db->set($data)->where('id',$where)->update($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_student_grade_files($where = false)
	{
