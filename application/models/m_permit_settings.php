...
class M_permit_settings Extends CI_Model
{
	private $_table = 'permit_settings';
	...
	public function create_permit_settings($input = false)
	{
		$this->db->insert($this->_table,$input);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function get_permit_settings($id = false)
	{
		$sql = "select *
				FROM $this->_table
				WHERE id = ?
	...
	public function update_permit_settings($data = false, $where = false)
	{
		$this->db->set($data)->where('id',$where)->update($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_permit_settings($where = false)
	{
