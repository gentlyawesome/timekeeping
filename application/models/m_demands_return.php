...
class M_demands_return Extends My_Model
{
	protected $_table = 'demands_return';

	public function create_demands_return($input = false)
	{
		$this->db->insert($this->_table,$input);
		return $this->db->affected_rows() > 0 ? array('status'=>'true','id'=>$this->db->insert_id()) : array('status'=>'false');
	}
	
	public function get_demands_return($id = false)
	{
		$sql = "select 
					*
				FROM $this->_table
	...
	public function get_demands_return_by_demand_id($id)
	{
		$sql = "select 
					*
				FROM $this->_table
	...
	public function update_demands_return($data = false, $where = false)
	{
		$this->db->set($data)->where('id',$where)->update($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_demands_return($where = false)
	{
