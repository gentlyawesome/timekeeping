...
class M_semesters Extends CI_Model
{
	...
	*/
	...
	public function create_semesters($input = false)
	{
	...
	public function update_semesters($data = false, $where = false)
	{
		$this->db->set($data)->where('id',$where)->update($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_semesters($where = false)
	{
