...
class M_librarycards Extends CI_Model
{
	private $_table = 'librarycards';
	...
	public function get_librarycards_bookid($id = false)
	{
		$sql = "select *
				FROM $this->_table
				WHERE library_id = ?
	...
	public function update_librarycards($data = false, $where = false)
	{
		$this->db->set($data)->where('id',$where)->update($this->_table);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function delete_librarycards($where = false)
	{
