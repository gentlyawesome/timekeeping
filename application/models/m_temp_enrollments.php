...
class M_temp_enrollments Extends CI_Model
{
	
	private $enrollment_tbl = 'temp_enrollments';
	private $profile_tbl = '';
	...
	public function get_by_studid($studid)
	{
		$query = $this->db->where('studid',$studid)->get($this->enrollment_tbl);
		return $query->num_rows() > 0 ? $query->row() : FALSE;
	}
	...
	public function profile($data)
	{
		$sql = "select 
		         temp_enrollments.*,
		         temp_enrollments.guardian as guardian_name,
		         temp_enrollments.fake_email as email,
		         bss.name as block_name,
					   temp_enrollments.fname as first_name,
					   temp_enrollments.name as full_name,
					   temp_enrollments.middle as middle_name,
					   temp_enrollments.lastname as last_name,
					   temp_enrollments.sex as gender,
					   temp_enrollments.id as enrollment_id,
					   courses.course,
					   years.year , 
					   years.id as year_id,
					   ...
					   users.login
				FROM temp_enrollments
				LEFT JOIN courses  ON courses.id = temp_enrollments.course_id
				LEFT JOIN years	  ON years.id = temp_enrollments.year_id
				LEFT JOIN semesters ON semesters.id = temp_enrollments.semester_id
				LEFT JOIN users ON users.id = temp_enrollments.user_id
				LEFT JOIN block_system_settings bss ON bss.id = temp_enrollments.block_system_setting_id
				WHERE temp_enrollments.id = ?";
		$query = $this->db->query($sql,array($data));
		// vd($this->db->last_query());
		return $query->num_rows() > 0 ? $query->row() : FALSE;
	}
	
	public function profile_by_temp_start_id($data)
	{
		$sql = "select 
		         temp_enrollments.*,
		         temp_enrollments.guardian as guardian_name,
				...
				LEFT JOIN semesters ON semesters.id = temp_enrollments.semester_id
				LEFT JOIN users ON users.id = temp_enrollments.user_id
				WHERE temp_enrollments.temp_start_id = ?";
		$query = $this->db->query($sql,array($data));
		// vd($this->db->last_query());
		return $query->num_rows() > 0 ? $query->row() : FALSE;
	}
	...
	public function profile_by_user_id($data)
	{
		$sql = "select 
				...
				LEFT JOIN semesters ON semesters.id = temp_enrollments.semester_id
				LEFT JOIN users ON users.id = temp_enrollments.user_id
				WHERE temp_enrollments.user_id = ?
				ORDER BY temp_enrollments.id DESC";
		$query = $this->db->query($sql,array($data));
		return $query->num_rows() > 0 ? $query->row() : FALSE;
	}
	...
	public function enrollee_profile($data)
	{
		$sql = "select concat(profiles.last_name,' , ',profiles.first_name,' ',profiles.middle_name) as stud_name,
					   ...
					   users.login
				FROM temp_enrollments
				LEFT JOIN profiles ON profiles.id = temp_enrollments.profile_id
				LEFT JOIN courses  ON courses.id = temp_enrollments.course_id
				LEFT JOIN years	  ON years.id = temp_enrollments.year_id
				LEFT JOIN semesters ON semesters.id = temp_enrollments.semester_id
				LEFT JOIN users ON users.id = temp_enrollments.user_id
				WHERE temp_enrollments.deleted = 0 AND temp_enrollments.id = ?";
		$query = $this->db->query($sql,array($data));
		return $query->num_rows() > 0 ? $query->row() : FALSE;
	}
	...
	{
		$sql = "select users.name, temp_enrollments.id
				FROM temp_enrollments
				LEFT JOIN users ON users.id = temp_enrollments.user_id
				WHERE temp_enrollments.is_deleted = 0 
				AND temp_enrollments.is_paid = 1 
				AND temp_enrollments.course_id = ?
				AND temp_enrollments.year_id = ?
				AND temp_enrollments.semester_id = ?
	...
	{
		$sql = "select users.name, temp_enrollments.id, semesters.name, temp_enrollments.sy_from, temp_enrollments.sy_to, temp_enrollments.status, temp_enrollments.studid, temp_enrollments.name, courses.course, years.year
				FROM temp_enrollments
				LEFT JOIN semesters ON semesters.id = temp_enrollments.semester_id
				LEFT JOIN users ON users.id = temp_enrollments.user_id
				LEFT JOIN courses ON courses.id = temp_enrollments.course_id
	...
	public function search_deleted_accounts($firstname, $lastname, $idno, $semester_id_eq, $sy_from_eq, $sy_to_eq)
	{
		...
		{
			$sql = "SELECT temp_enrollments.id, temp_enrollments.studid, profiles.first_name, profiles.last_name, years.year, semesters.name, courses.course FROM temp_enrollments 
			LEFT JOIN profiles ON (temp_enrollments.profile_id = profiles.id) 
			LEFT JOIN years ON (years.id = temp_enrollments.year_id) 
			LEFT JOIN semesters ON (semesters.id = temp_enrollments.semester_id)
			LEFT JOIN courses ON (courses.id = temp_enrollments.course_id)
			WHERE profiles.first_name LIKE '%$firstname%' AND temp_enrollments.semester_id='$semester_id_eq' AND temp_enrollments.sy_from='$sy_from_eq' AND temp_enrollments.sy_to='$sy_to_eq'  AND temp_enrollments.is_deleted = 1";
		}
		elseif(!empty($lastname) && $lastname!='')
		{
			...
			LEFT JOIN semesters ON (semesters.id = temp_enrollments.semester_id)
			LEFT JOIN courses ON (courses.id = temp_enrollments.course_id)
			WHERE profiles.last_name LIKE '%$lastname%' AND temp_enrollments.semester_id='$semester_id_eq' AND temp_enrollments.sy_from='$sy_from_eq' AND temp_enrollments.sy_to='$sy_to_eq'  AND temp_enrollments.is_deleted = 1";
		}
		elseif(!empty($idno) && $idno!='')
		{
			...
			LEFT JOIN semesters ON (semesters.id = temp_enrollments.semester_id)
			LEFT JOIN courses ON (courses.id = temp_enrollments.course_id)
			WHERE temp_enrollments.studid LIKE '%$idno%' AND (temp_enrollments.semester_id='$semester_id_eq' AND temp_enrollments.sy_from='$sy_from_eq' AND temp_enrollments.sy_to='$sy_to_eq' AND temp_enrollments.is_deleted = 1)";
		}
		else
		{
			$sql = "SELECT temp_enrollments.id, temp_enrollments.studid, profiles.first_name, profiles.last_name, years.year, semesters.name, courses.course FROM temp_enrollments 
			...
			LEFT JOIN semesters ON (semesters.id = temp_enrollments.semester_id)
			LEFT JOIN courses ON (courses.id = temp_enrollments.course_id)
			WHERE temp_enrollments.semester_id='$semester_id_eq' AND temp_enrollments.sy_from='$sy_from_eq' AND temp_enrollments.sy_to='$sy_to_eq' AND temp_enrollments.is_deleted = 1";
		}
	...
	{
		$sql = "SELECT temp_enrollments.id, temp_enrollments.studid, profiles.first_name, profiles.last_name, years.year, semesters.name, courses.course FROM temp_enrollments 
			LEFT JOIN profiles ON (temp_enrollments.id = profiles.enrollment_id) 
			LEFT JOIN years ON (years.id = temp_enrollments.year_id) 
			LEFT JOIN semesters ON (semesters.id = temp_enrollments.semester_id)
			LEFT JOIN courses ON (courses.id = temp_enrollments.course_id)
			WHERE temp_enrollments.semester_id='$semester_id_eq' AND temp_enrollments.sy_from='$sy_from_eq' AND temp_enrollments.sy_to='$sy_to_eq' AND temp_enrollments.is_paid=0
			AND temp_enrollments.deleted = 0";
	...
	public function get_enrollee_bydate($semester_id_eq, $sy_from_eq, $sy_to_eq, $date)
	{
		$sql = "SELECT 
					...
					temp_enrollments.is_paid, 
					temp_enrollments.name as fullname,
					years.year, 
					semesters.name, 
					courses.course 
				FROM temp_enrollments 
				...
				LEFT JOIN semesters ON (semesters.id = temp_enrollments.semester_id)
				LEFT JOIN courses ON (courses.id = temp_enrollments.course_id)
				WHERE temp_enrollments.semester_id='$semester_id_eq' 
				AND temp_enrollments.sy_from='$sy_from_eq' 
				AND temp_enrollments.sy_to='$sy_to_eq' 
				AND (DATE(temp_enrollments.created_at) = ?) 
				AND temp_enrollments.is_paid = 0
	...
	public function update_enrollments_by_temp_start_id($data = false, $where = false)
	{
		$this->db->set($data)->where('temp_start_id',$where)->update($this->enrollment_tbl);
		return $this->db->affected_rows() > 0 ? array('status'=>'true') : array('status'=>'false');
	}
	
	public function create_enrollments($input = false)
	{
		$this->db->insert("temp_enrollments",$input);
	...
	public function destroy_enrollment_temp_start_id($id)
	{
		$this->db->where('temp_start_id',$id)->delete($this->enrollment_tbl);
		return $this->db->affected_rows() > 0 ? TRUE : FALSE;
	}
	...
	public function view_grade($enrollment_id,$data)
	{
		...
		$this->db->update('temp_enrollments', $datus); 
		return $this->db->affected_rows() > 0 ? TRUE : FALSE;
	}
	...
	{
	  $ci =& get_instance();
	  $sql = "SELECT temp_enrollments.id, temp_enrollments.name, temp_enrollments.studid
	          FROM studentsubjects
	          JOIN temp_enrollments on studentsubjects.enrollmentid = temp_enrollments.id
   ...
   public function get_all_student_profile_for_report($filter=false, $fields = false)
   {
		...
		//GET temp_enrollments FIELDS
		// $sql = "DESCRIBE temp_enrollments";
	...
	public function find($start=0,$limit=100, $filter = false, $all = false, $ret_count = false, $like = false){
		...
		//ADD FILTERS
		$fields_array[] = 'temp_enrollments.id';
		$fields_array[] = 'temp_enrollments.studid';
		$fields_array[] = 'temp_enrollments.name';
		$fields_array[] = 'courses.course';
		$fields_array[] = 'years.year';
		...
		$this->db->order_by("temp_enrollments.name", "ASC"); 
		$this->db->join('courses', 'courses.id = '.$this->enrollment_tbl.'.course_id','LEFT');
		$this->db->join('years', 'years.id = '.$this->enrollment_tbl.'.year_id','LEFT');
	...
	{
		//CHECK IF THERE IS temp_enrollments SET AS DEFAULT (temp_enrollments.is_used = 1) IF NONE UPDATE
		$sql = "
			SELECT id 
			FROM $this->enrollment_tbl
			WHERE studid = ?
	...
	public function reset_by_studid($studid)
	{
		$this->db->where('studid',$studid)->delete($this->enrollment_tbl);
		return $this->db->affected_rows() > 0 ? TRUE : FALSE;
	}
	
	public function reset_by_temp_start_id($id)
	{
		$this->db->where('temp_start_id',$id)->delete($this->enrollment_tbl);
		return $this->db->affected_rows() > 0 ? TRUE : FALSE;
	}
